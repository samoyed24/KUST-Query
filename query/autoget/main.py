...
def query(type, username, password):
    s = requests.Session()
    ...
    if type == "grade":
        return gradeQuery(s, username)
    elif type == "table":
        return tableGet(s, username)
    else:
        raise

def gradeQuery(s, username):
